...
class CoursePriceSerializer(serializers.Serializer):
    id = serializers.IntegerField()
    name = serializers.CharField()
    course_price = serializers.SerializerMethodField()

    def get_course_price(self,row):
        course_price = row.coursedetail.recommend_courses.all()

        return [{'id':item.id,'name':item.name} for item in course_price ]


class CourseModelSerializer(serializers.ModelSerializer):
    level_name = serializers.CharField(source='get_level_display')
    # 一对一，可以使用source ，跨表获取字段
